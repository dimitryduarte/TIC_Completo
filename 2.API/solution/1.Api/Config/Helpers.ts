...
import http, { IncomingMessage, ClientRequestArgs } from "http";
...
export default class Helpers
{
    ...
    public Request(url: string, params: ClientRequestArgs = {}): JSON
    {
        let data = "";

        if(params)
        {
            http.request(url, params, function(res: IncomingMessage) {
        ...
        {
            http.request(url, function(res: IncomingMessage) {
                // A chunk of data has been recieved.
                res.on("data", (chunk) => {
                    data += chunk;
                });
        ...
        try
        {
            if(_req.headers["authorization"])
            {
                if(_req.headers["authorization"] == await this.GeraToken())
                {
                    ...
                    _res.json(new ReturnMessage<null>(400, "Token inválido !!!", null));
            }
            else
                _res.json(new ReturnMessage<null>(400, "Token não encontrado !!!", null));
        }
        catch(ex)
        {
            _res.json(new ReturnMessage<null>(500, ex.toString(), null));
        }
    }
